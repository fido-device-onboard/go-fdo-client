...
declare -a directories=("${base_dir}" "${certs_dir}" "${credentials_dir}" "${logs_dir}" "${pid_dir}")
...
run_go_fdo_server() {
  ...
  nohup "go-fdo-server" "${role}" "${address_port}" --db-type sqlite --db-dsn "file:${base_dir}/${name}.db" --debug "${@}" &> "${log}" &
  echo -n $! > "${pid_file}"
}
