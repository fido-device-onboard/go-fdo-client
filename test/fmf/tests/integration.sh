...
log_info "Install go-fdo-server"
sudo cat > /etc/yum.repos.d/fedora-iot.repo << 'REPO_EOF'
...
enabled_metadata=1
REPO_EOF
sudo dnf install -y go-fdo-server
...
# VM management
log_info "Provisioning edge device..."
...
if [[ -z "${guid}" || "${guid}" == "null" ]]; then
    log_error "Failed to retrieve GUID from manufacturer."
    exit 1
fi
